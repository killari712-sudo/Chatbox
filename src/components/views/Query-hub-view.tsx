...
export function QueryHubView() {
    ...
    useEffect(() => {
        // This effect will run whenever `queries` state changes, ensuring the UI is up-to-date.
    ...
    useEffect(() => {
        ...
        const handleDocumentClick = (e: MouseEvent) => {
            ...
            const card = target.closest('.frosted-card[data-query-id]');
    ...
    }, [queries]); // Rerun this setup if the queries array reference changes.

    function renderQueries(queriesToRender: any[]) {
        const queryFeed = document.getElementById('query-feed');
        if (!queryFeed) return;
        queryFeed.innerHTML = ''; // Clear existing content
    ...
    function renderQueryCard(query: any) {
        ...
        card.className = 'frosted-card p-6 rounded-2xl flex flex-col space-y-4 hover:border-gray-500 transition-colors fade-slide-up';
        card.setAttribute('data-query-id', query.id);
        ...
        cardHeader.innerHTML = `
            ...
            <div class="flex-1">
                ...
                <div class="text-sm text-gray-500 flex items-center space-x-2">
                    ...
                    ${query.mood ? `<span class="text-xl ml-2" title="Feeling ${query.mood}">${{'anxious': '😥', 'stressed': '😰', 'sad': '😔', 'happy': '😊'}[query.mood]}</span>` : ''}
                </div>
            </div>
        `;
        ...
        <div className='query-hub-body'>
                ...
                <div className="frosted-card p-8 rounded-2xl max-w-lg w-full text-center border-red-300">
                    ...
                    <div className="flex flex-col space-y-4">
                        <button className="w-full bg-red-600 text-white font-semibold py-3 px-6 rounded-xl transition-all hover:bg-red-700">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-message-square-heart inline-block mr-2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/><path d="M17.8 10c-1.5 1.5-3 3-5.8 3-2.8 0-4.3-1.5-5.8-3-1.5-1.5 0-4.5 5.8-4.5 5.8 0 7.3 3 5.8 4.5Z"/></svg> Counselor Chat
                        </button>
                        <button className="w-full bg-gray-400 text-gray-800 font-semibold py-3 px-6 rounded-xl transition-all hover:bg-gray-500">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-phone-call inline-block mr-2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/><path d="M14.05 2a9 9 0 0 1 8 7.94"/><path d="M14.05 6A5 5 0 0 1 18 10"/></svg> SOS Helpline
                        </button>
                        <button className="w-full bg-blue-600 text-white font-semibold py-3 px-6 rounded-xl transition-all hover:bg-blue-700">
                           <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-leaf inline-block mr-2"><path d="M11 20A7 7 0 0 1 4 13V8a7.99 7.99 0 0 1 14 0v5a7 7 0 0 1-7 7z"/><path d="M12 18c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4z"/></svg> Quick Coping Exercise
                        </button>
                    </div>
                    ...
                    <header className="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 mb-8 p-4 rounded-2xl frosted-card">
                        ...
                        <div className="flex items-center space-x-2 w-full md:w-auto">
                            ...
                            <button id="ask-button" className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold py-2 px-6 rounded-full transition-transform transform hover:scale-105 ripple-effect">
                               <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-plus inline-block mr-1"><line x1="12" x2="12" y1="5" y2="19"/><line x1="5" x2="19" y1="12" y2="12"/></svg> Ask
                            </button>
                        </div>
                    </header>
            ...
            <div id="new-query-modal" className="hidden fixed inset-0 z-50 flex items-center justify-center p-4 modal-overlay">
                <div className="frosted-card p-8 rounded-2xl max-w-2xl w-full">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button onClick={() => (window as any).hideModal('new-query-modal')} className="text-gray-400 hover:text-gray-600">
            ...
            <div id="query-expanded-modal" className="hidden fixed inset-0 z-50 items-center justify-center p-4 modal-overlay">
                <div className="frosted-card p-8 rounded-2xl max-w-4xl w-full h-full lg:max-h-[90vh] overflow-y-auto relative">
                    <button onClick={() => (window as any).hideModal('query-expanded-modal')} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 z-10">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x"><line x1="18" x2="6" y1="6" y2="18"/><line x1="6" x2="18" y1="6" y2="18"/></svg>
                    </button>
